...
#if FFMPEG_UPDATED
                opaque->bsfc = av_bsf_get_by_name("h264_mp4toannexb");
#else
...
#if FFMPEG_UPDATED
                opaque->bsfc = av_bsf_get_by_name("hevc_mp4toannexb");
#else
...
static int feed_input_buffer(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs, int *enqueue_count)
{
    ...
    if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
        ...
        d->bfsc_ret =
            av_bsf_send_packet(opaque->bsfc, &d->pkt);
        if (0 > d->bfsc_ret) {
            ALOGE("%s: av_bsf_send_packet failed %d:%s\n", __func__, d->bfsc_ret, av_err2str(d->bfsc_ret));
            ret = -1;
            goto fail;
        }

        d->bfsc_ret = av_bsf_receive_packet(opaque->bsfc, &d->pkt_temp);
        if (0 < d->bfsc_ret) {
            d->bfsc_data = d->pkt_temp.data;
        } else if (0 > d->bfsc_ret) {
            ALOGE("%s: av_bsf_receive_packet failed %d:%s\n", __func__, d->bfsc_ret, av_err2str(d->bfsc_ret));
            ret = -1;
            goto fail;
        }
...
#if FFMPEG_UPDATED
        ret = ffp_queue_picture(ffp, frame, pts, duration, 0, is->viddec.pkt_serial);
#else
